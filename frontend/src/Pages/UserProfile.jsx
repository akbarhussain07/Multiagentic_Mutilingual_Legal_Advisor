...
const UserProfile = () => {
  ...
  const handleDelete = async () => {
    const confirmDelete = window.confirm(
      "ARE YOU SURE? This will permanently delete this user and all history."
    );
    if (confirmDelete) {
      try {
        await deleteUser(userId);
  ...
  return (
    <div className="profile-shell">
      ...
      {sidebarOpen && (
        <div className="profile-sidebar-overlay visible" onClick={() => setSidebarOpen(false)} />
      )}
            ...
            {selectedSession.messages?.map(msg => (
              <div key={msg.message_id} className={`message-row ${msg.role}`}>
                ...
                {msg.role === 'user' ? (
                  ...
                  /* Assistant Message: Split Rendering for Dual Law */
                  <div className="bot-dual-wrapper">
                    
                    {/* Pakistani Law Section */}
                    {(msg.content?.pakistanContent || (!msg.content?.islamicContent && msg.content?.answer)) && (
                      <div className="message-bubble pak-bubble">
                        <div className="role-label">Pakistani Law</div>
                        <div className="content">
                          {msg.content?.pakistanContent || msg.content?.answer || msg.content}
                        </div>
                        <div className="msg-time bot-time">
                           {new Date(msg.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                        </div>
                      </div>
                    )}
                    ...
                    {/* Islamic Law Section */}
                    {msg.content?.islamicContent && (
                      <div className="message-bubble isl-bubble">
                        <div className="role-label">Islamic Law</div>
                        <div className="content">{msg.content.islamicContent}</div>
                        <div className="msg-time bot-time">
                          {new Date(msg.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                        </div>
                      </div>
